 ...
 */
static void unix_signal_forward(int sig) {
#ifdef _WIN32
	g_assert_not_reached(); /* shouldn't be tried to be called on win32 */
#else
	...
	for (;;) {
		...
		while (nprocs < 1) {
			...
			if (pid == 0) {
				...
				signal(SIGINT, unix_signal_forward);
				signal(SIGTERM, unix_signal_forward);
				signal(SIGHUP, unix_signal_forward);
				signal(SIGUSR1, unix_signal_forward);
				signal(SIGUSR2, unix_signal_forward);
		...
		if (child_pid != -1) {
			...
			if (exit_pid == child_pid) {
				...
				if (WIFEXITED(exit_status)) {
					...
					g_critical("%s: [angel] PID=%d died on signal=%d (it used %ld kBytes max) ... waiting %d sec before restart",
							G_STRLOC,
							child_pid,
