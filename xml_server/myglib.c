 ...
 * @see myglib_log_t_new()
 */
myglib_log_t *myglib_log_init(void) {
	return myglib_log_new();
}

myglib_log_t *myglib_log_new(void) {
	myglib_log_t *log;

	log = g_new0(myglib_log_t, 1);

	log->log_file_fd = -1;
	log->log_ts_str = g_string_sized_new(sizeof("2004-01-01T00:00:00.000Z"));
	log->log_ts_resolution = MYGLIB_RESOLUTION_DEFAULT;
...
//add by vinchen/CFR
char* myglib_log_get_level_name(int	log_level) {
	gint	i;
 ...
 * @return 0 on success
 *
 * @see myglib_log_t_open
 */
int myglib_log_close(myglib_log_t *log) {
...
static int myglib_log_update_timestamp(myglib_log_t *log) {
	struct tm *tm;
	GTimeVal tv;
	time_t	t;
	...
	if (log->log_ts_resolution == MYGLIB_RESOLUTION_MS)
...
void myglib_log_set_logtimestamp_resolution(myglib_log_t *log, int res) {
	...
	if (res == MYGLIB_RESOLUTION_SEC || res == MYGLIB_RESOLUTION_MS)
 ...
 *
 * <absolute-path>/src/MYGLIB-log.c will become src/MYGLIB-log.c
 *
...
void myglib_log_func(                 
    const gchar *UNUSED_PARAM(log_domain), 
    GLogLevelFlags      log_level, 
    const gchar *       message, 
    gpointer            user_data
) 
{
	myglib_log_t *log = user_data;
	int i;
	gchar *log_lvl_name = "(error)";
	gboolean is_duplicate = FALSE;
	gboolean is_log_rotated = FALSE;
	const char *stripped_message = myglib_log_skip_topsrcdir(message);
	...
	if (is_log_rotated ||
		...
		if (log->last_msg_count) {
			myglib_log_update_timestamp(log);
			g_string_append_printf(log->log_ts_str, ": (%s) last message repeated %d times",
					log_lvl_name,
					log->last_msg_count);

			myglib_log_write(log, log_level, log->log_ts_str);
		}
		myglib_log_update_timestamp(log);
